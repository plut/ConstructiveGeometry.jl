...
"""
		Solid.include(file::AbstractString, f::Function)

Reads given `file` and returns the union of all top-level `Solid` objects
...
julia> Solid.include("example.jl")
union() {
 circle(radius=3.0);
...
Read top-level expression `E` and decides if a Solid object is returned.
